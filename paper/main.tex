...
runs on commodity CPU hardware. All experiments use CRAG (English), BM25 retrieval,
and a uniform-word-stream proxy for input cadence; speech is the motivating
application but is not directly tested. We find that at a realistic operating
point ($L{=}600$\,ms, $\delta{=}3$\,w/s, $\theta{=}0.8$), \streamPct{} of queries
across the full benchmark admit substantial latency hiding---a blended figure that
mixes sufficiency stabilization on the \nRetrPct{} of questions where gold evidence
is verbatim-present and BM25-retrievable (\streamSufPct{} streamable on this
favorable slice) with a grounding-free top-1-settling fallback on the remainder.
On the favorable slice, $\phi_{\mathrm{suf}}$ is bracketed to
$[\phiSufMean{},\,\phiSufFuzzy{}]$ by exact and relaxed grounding---both early.
Question type produces a significant but coarse early/late split
(Kruskal--Wallis $p=\kwAllP{}$, $\varepsilon^2{=}0.04$), directly informing when
a learned speculative trigger is worth its cost.
\end{abstract}
...
\item[H3 (RQ4).] CRAG question type predicts $\phi$: simple lookups stabilize
  ...
  stabilize late.
\end{description}
We test H1--H3 directly (\S5), and we report a top-$k$ robustness sweep
(\S\ref{sec:method}, Table~\ref{tab:topk}) as a within-retriever sensitivity
check. Dense-retriever and cross-linguistic conditions are left to future work.
We report outcomes
