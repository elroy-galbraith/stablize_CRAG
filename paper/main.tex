...
\newcommand{\hPred}{590}
\newcommand{\rqThreeRho}{-0.06}      % Spearman(residual input time, measured saving) %
...
evidence is both verbatim-present and retrievable), that \streamPct{} of queries
admit substantial latency hiding at a realistic operating point---\streamSufPct{}
of the retrieved-gold subset and \streamScPct{} of the remainder---and that
question type is an ordered predictor of stabilization (Kruskal--Wallis
$p=\kwAllP{}$), directly informing when a learned speculative trigger is worth its
cost.
\end{abstract}
...
\paragraph{Hypotheses.} The study is organized around four pre-registered
...
Table~\ref{tab:topk}) as a within-retriever sensitivity check.
...
is high at realistic operating points. Two limitations temper these claims and set
the agenda for follow-up: the sufficiency results are conditional on a favorable
$\nRetrPct{}$ slice of the benchmark, and our system validation establishes
conservativeness in aggregate but not per-query prediction, because the bound
saturates at the operating point we replayed (\S5.4). A higher-$L$ replay, a
dense-retriever condition (H4), and a regression of $\phi$ on query features are
