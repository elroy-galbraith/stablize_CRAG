...
itself bias stabilization early, and a dense-retriever replication is the main open
confound. On the favorable slice, $\phi_{\mathrm{suf}}$ is bracketed to
...
We address RQ1--RQ4 in \S5 and include a top-$k$ robustness sweep
(Table~\ref{tab:topk}) as a sensitivity check. Dense-retriever and
cross-linguistic conditions are left to future work. We report
...
dense-retriever condition and a regression of $\phi$ on query features are the
...
low $\phi_{\mathrm{suf}}$ we report---a dense or learned retriever weights terms
differently and might surface gold later (or earlier). This is the principal
confound on our central early-stabilization claim; our two grounding arms bound the
\emph{population}-selection bias (\S\ref{sec:robust}) but not this
\emph{retriever}-induced one. A dense-retriever condition and a second QA set are
therefore the natural---and, for a methods-heavy venue, close to required---next
steps, not optional polish.
