...
\paragraph{Stabilization over a growing prefix.} The core object we study---when a
prediction over a growing input prefix becomes safe to commit---is the central
...
tension. Incremental dialogue processing~\cite{schlangen2011incremental} formalizes
the same prefix-monotonicity and revision concerns (its notion of an output that
later input may \emph{revise} is our volatility $V$). Our contribution is to port
this lens to \emph{tool/retrieval} intent: rather than learning a commit policy, we
...
\end{description}
We test H1--H3 directly (\S5); the dense-retriever condition for H4 is left to
future work, and we instead report a top-$k$ robustness sweep (\S\ref{sec:method},
Table~\ref{tab:topk}) as a within-retriever sensitivity check. We report outcomes
...
overlap and a mis-fire penalty---together with a dense-retriever condition (H4) and a
regression of $\phi$ on query features are the natural next steps. Even so, the present results give system builders a
